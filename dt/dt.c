...
void dt_recur(Row **rows, Tree *tree, int nc, int nr, char *type) {
	...
	// fprintf(stderr, "2/3 column is: %d\n", num_use); ///////////////
	...
	for(int w = 0; w < num_use; w++) {
		...
		// fprintf(stderr, "Check the column: %d\n", i); ///////////////
		
		if(type[i] == 'D') {
			//double* fields = (double*)calloc(nr, sizeof(double)); 
			for(int j = 0; j < nr; j++) {
				d_fields[j] = tbl_double_at( rows[j], i );
			}
			Column *columns = make_double_column(d_fields, classes, nr);
			//free(fields);
			if(is_impossible_split(columns) || has_single_value(columns)) {
				free_column(columns);
				continue;
			}
			double d_entropy = find_double_split_entropy(columns);
			...
			for(int j = 0; j < nr; j++) {
				s_fields[j] = tbl_string_at( rows[j], i );
			}
			Column *columns = make_string_column(s_fields, classes, nr);
			//free(fields);
			if(is_impossible_split(columns) || has_single_value(columns)) {
				free_column(columns);
				continue;
			}
			double s_entropy = find_string_split_entropy(columns);
