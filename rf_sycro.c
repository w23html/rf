...
int main(int argc, char **argv) {
	...
	if(num_threads != 0) {
		...
		T_arg **t_arg = (T_arg**)calloc(nth, sizeof(T_arg*));
		int thread_index = (int)(ntr/nth);
		int sum_index = 0;
		Row **train_tbl_rows = tbl_rows(train_budtbl);
		
		for(int q = 0; q < nth; q++) {
			t_arg[q] = (T_arg*)calloc(1, sizeof(T_arg));
			t_arg[q]->tbl = train_budtbl;
			t_arg[q]->rows = train_tbl_rows;
			t_arg[q]->trees = ntree;
			t_arg[q]->from = sum_index;
			sum_index = sum_index + thread_index;
			t_arg[q]->to = sum_index;
		}
		t_arg[nth-1]->to = num_trees;
	
		for(int w = 0; w < nth; w++) {
			pthread_create(&(thread[w]), &attr, build_trees, t_arg[w] );
		}
		for(int w = 0; w < nth; w++) {
